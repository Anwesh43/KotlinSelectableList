 ...
 */
class SelectableView(var ctx:Context,var text:String,var handler:AnimHandler,vararg var listeners: OnSelectedListener):View(ctx) {
    var time = 0
    ...
    fun stopped():Boolean {
        var condition = selectableItem?.stoppedUpdating()?:true
        if(condition) {
